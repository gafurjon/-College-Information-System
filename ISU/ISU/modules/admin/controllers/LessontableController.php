...
class LessontableController extends \yii\web\Controller
{
    ...
    public function actionLessonsTable()
    {
        ...
        $groups = GroupStudents::find()->where(['group_students.id_group' => $id_group])->
        joinWith('profession')->asArray()->all();
        ...
        joinWith('lessonTime.timeLesson')->where('status=1')->all();
        ...
        for ($i = 1; $i < 7; $i++) {
            ...
            for ($j = 1; $j <= count($timelesson); $j++) {
                ...
                where(['lesson_time.id_time_lesson' => $j,
                    'lesson_time.id_week' => $i, 'lessons_table.id_group' => $id_group, 'lessons_table.status'=> 1])->one();
